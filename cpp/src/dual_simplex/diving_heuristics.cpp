                                         ...
                                         logger_t& log)
{
  ...
  const i_t n = pc.pseudo_cost_sum_down.size();
  auto result = pseudocost_diving_from_arrays(pc.pseudo_cost_sum_down.data(),
  ...
  assert(result.direction != rounding_direction_t::NONE);
  assert(result.variable >= 0);

  log.debug("Pseudocost diving: selected var %d with val = %e, round dir = %d\n",
            result.variable,
                                     ...
                                     logger_t& log)
{
  ...
  const i_t n = pc.pseudo_cost_sum_down.size();
  auto result = guided_diving_from_arrays(pc.pseudo_cost_sum_down.data(),
                                          pc.pseudo_cost_sum_up.data(),
                                          pc.pseudo_cost_num_down.data(),
                                          pc.pseudo_cost_num_up.data(),
                                          n,
  ...
  log.debug("Guided diving: selected var %d with val = %e, round dir = %d\n",
            result.variable,
            solution[result.variable],
            result.direction);
